...
contract AuthenticationContract{
    ...
    // một fog node có thể có nhiều devices
    ...
    // add 1 device cho một user , điều kiện là device đó phải tồn tại trong fog node đã chọn
    ...
    // xóa tất cả device mà user được chọn đang sở hữu
    ...
    // xóa tất cả devices ở một fog node được chọn
    ...
    event TokenProcessedEvent(bytes32 uid,address user, bool access);
}